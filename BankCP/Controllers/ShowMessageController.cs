...
{
    [RoutePrefix("api/ShowMessage")]
    [Authorize]
    public class ShowMessageController : ApiController
    {
        ...
        public IHttpActionResult get(int branchId, int screenId)
        {
            try
            {
                ...
                List<CustomShowMessageButton> lstButtons = bALButton.selectShowMessagebyBranchIdAnsScreenId(bankId, branchId, screenId);
                if (lstButtons == null)
                {
